...
const tabImgitem = document.querySelectorAll(".tab__imgitem");
const tabImgcontent = document.querySelectorAll(".tab__imgcontent");

// 2. 탭 버튼들을 forEach 문을 통해 한번씩 순회한다.
// 이때 index도 같이 가져온다.
tabImgitem.forEach((item, index) => {
    // 3. 탭 버튼에 클릭 이벤트를 준다.
    item.addEventListener("click", (e) => {
        ...
        // active 클래스가 추가된다.
        tabImgitem[index].classList.add("active");
        tabImgcontent[index].classList.add("active");
    });
});